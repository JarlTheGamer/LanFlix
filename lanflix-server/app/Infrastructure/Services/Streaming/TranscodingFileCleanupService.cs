...
public class TranscodingFileCleanupService
{
        ...
        ILogger<TranscodingFileCleanupService> logger)
    {
        ...
        // Ensure temp directory exists - create all parent directories if needed
        try
        {
        ...
        {
            // If configured path fails, fall back to system temp
            _logger.LogWarning(ex, "Failed to create configured temp path {Path}, falling back to system temp", _tempPath);
            _tempPath = Path.Combine(Path.GetTempPath(), "Lanflix", "Transcoding");
            Directory.CreateDirectory(_tempPath);
            _logger.LogInformation("Using fallback temp directory: {Path}", _tempPath);
        }
    }
